...
public abstract class AbstractTcpServer extends Thread {
	...
	/**
	 * The abstract method, which will be called in
	 * {@link AbstractTcpServer#waitForClients()}
	 * 
	 * @param aClientSocket
	...
	private void bindOnSelectedInterface() throws IOException {
		...
		while (inetAddList.hasMoreElements()) {
			...
			System.out.println(
					this.getClass().getName() + " is bound into " + addr.getHostAddress() + " on " + serverPort);
		}
	}
	 ...
	 * - a cleanup thread (if it was not started) -> reduce handled clients
	 * (sockets).<br />
	 */
	...
	/**
	 * The cleanup part of the server , this started by the Thread.start()
	 * command.<br/>
	 * DO NOT CALL directly the "start()" :
	 * {@link AbstractTcpServer#waitForClients()}
	 */
	@Override
	public void run() {
